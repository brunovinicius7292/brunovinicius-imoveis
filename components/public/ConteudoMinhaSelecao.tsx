...
import BotaoCompartilharSelecao from "@/components/public/BotaoCompartilharSelecao";
...
export default function ConteudoMinhaSelecao() {
  const { selecionados, removerDaSelecao, limparSelecao } = useSelecao();
  const [imoveis, setImoveis] = useState<Imovel[]>([]);
  const [carregando, setCarregando] = useState(true);

  function handleLimparSelecao() {
    const confirmar = window.confirm(
      "Tem certeza que deseja limpar a seleção? Isso remove todos os imóveis da sua seleção atual."
    );
    if (!confirmar) return;
    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        <a
          ...
          className="flex items-center justify-center gap-2 rounded-xl bg-[#25D366] py-3 font-body text-sm font-semibold text-white transition hover:brightness-95 sm:w-auto sm:px-8"
        >
        ...
        <BotaoCompartilharSelecao imovelIds={imoveis.map((imovel) => imovel.id)} />
        <button
          type="button"
          onClick={handleLimparSelecao}
          className="flex items-center justify-center gap-2 rounded-xl border border-navy-200 py-3 font-body text-sm font-semibold text-navy-500 transition hover:border-red-300 hover:text-red-600 sm:w-auto sm:px-8"
        >
          Limpar seleção
        </button>
      </div>
